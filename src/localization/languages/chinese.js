...
export default {
    ...
    [KEYS.BUTTON_DestroyArkId]:'删除 ArisenID',
    [KEYS.BUTTON_CreateNewArkId]:'创建新的 ArisenID',
    [KEYS.BUTTON_LoadFromBackup]:'从备份中恢复',
    [KEYS.BUTTON_Unlock]:'解锁',
    [KEYS.BUTTON_ExportArkId]:'导出 ArisenID',
...
     `,
    ...
    [KEYS.HISTORIES_Description]:`
        在此您能看到所有 ArisenID 的事件列表.
        但在您的 ArisenID 外的帐户上发生的事件不会显示.
     `,
    [KEYS.HISTORIES_Note]:`
        注意：当从设置面板中的备份选项中导出您的 ArisenID 数据时，不会备份这些事件。
    ...
    [KEYS.BACKUP_Description]:`
        导出您的 ArisenID 后，您将可以将其导入其他设备。导出文件将被加密，
    ...
    [KEYS.IMPORT_Description]:`
        导入你的加密钥匙串文件将重建您的 ArisenID 钥匙串，但是它不会导入您原有的网络
        或帐户设置
    `,
    ...
    [KEYS.DESTROY_Header]:`销毁 ArisenID`,
    [KEYS.DESTROY_Description]:`
        您即将销毁您的整个 ArisenID 钥匙串。恢复 ArisenID 的唯一方法是
        导入以前导出的 ArisenID JSON 文件。 否则你将没有办法再使用你的身份。
        请确保在着手这个操作前，已经从备份设置面板中导出了您的 ArisenID。
    `,
    ...
    [KEYS.KEYPAIR_Description]:`
        您可以通过此界面来生成 RSN 密钥对。密钥对是在你本地机器上生成并且不会被传送到其它任何地方。
    ...
    [KEYS.LOCK_Description]:`
        自动锁定能为您自动锁定 ArisenID，
        这样你就不必在离开时记得锁定您的 ArisenID。
    `,
    [KEYS.LOCK_Minute]:`分`,
    [KEYS.LOCK_Minutes]:`分`,
    [KEYS.LOCK_Hour]:`小时`,
    ...
    [KEYS.MNEMONIC_Note]:`
        ...
        这是恢复对 ArisenID 的访问和解密您的个人信息的唯一途径。
    `,
    ...
    [KEYS.ALERT_NetworkHostInvalid]:[
        ...
        `您输入的主机无效。
         主机应该为域名( 例: greatchain.arisennodes.io ) 或者 IP 地址 ( 如: 192.168.0.1 ).`
    ],
    //2
...
         您确定想将此列入白名单吗？`
    ...
    [KEYS.REQUEST_AddNetwork]:[
        /*{DOMAIN}*/ '想要将他们的网络添加到 ArisenID。',

        '一些应用使用它们自己的网络。',

        `应用决不会被允许访问您的 ArisenID。
         通过此界面添加网络。
         仅是为了比在控制面板中手工添加更方便`
    ],
    //5
...
         此事务的某些属性的更改，并且仅当其他属性发生更改时，
    ...
    [KEYS.REQUEST_UpdateVersion]:[
        ...
        `注意，我们让应用能够检查 ArisenID 的版本，但它们有可能让你下载恶意程序，请确保从正确的地址下载 ArisenID.`
    ],
